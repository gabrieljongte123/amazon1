...
def _detect_action_command(text: str, session: Session = None) -> dict | None:
    ...
    if any(p in text_lower for p in need_patterns):
        return _handle_need_prediction(text_lower)
    
    # ACCESSORY / COMPANION RECOMMENDATIONS — "what goes well with X", "suggest for X"
    companion_patterns = [
        r"suggest\s+what\s+to\s+go\s+with\s+(.+)",
        r"what\s+(?:would you|do you|can you)\s+suggest\s+(?:to go|for|with)\s+(.+)",
        r"suggest\s+(?:what to go|what goes|things? to go|things? that go)\s+with\s+(.+)",
        r"(?:what|which)\s+(?:goes|pairs?|to go)\s+(?:well\s+)?with\s+(.+)",
        r"(?:what|which)\s+(?:should i|can i)\s+(?:buy|get)\s+with\s+(.+)",
        r"(?:suggest|recommend)\s+(?:something|products?|items?)\s+(?:to go|that go|for)\s+(.+)",
        r"(?:accessories|companion|complementary)\s+(?:for|to)\s+(.+)",
        r"what else\s+(?:do i need|should i get)\s+(?:for|with)\s+(.+)",
        r"(?:what|anything)\s+(?:to|that)\s+(?:go|pair|match)(?:es)?\s+with\s+(.+)",
        r"(?:what would you|what do you|what can you)\s+(?:suggest|recommend)\s+.*?(?:with|for)\s+(.+)",
    ]
    for pattern in companion_patterns:
        match = re.match(pattern, text_lower)
        if match:
            base_product = match.group(1).strip().rstrip("?!.")
            # Clean up captured text - remove leading "with" if present
            if base_product.startswith("with "):
                base_product = base_product[5:].strip()
            # Map common products to their accessories (category-aware)
            accessory_map = {
                ...
                "iphone": ["iPhone case", "AirPods", "MagSafe charger", "screen protector", "lightning cable"],
                "camera": ["camera bag", "memory card", "tripod", "lens cleaner", "extra battery"],
                "bicycle": ["helmet", "cycling gloves", "water bottle", "bike lock", "pump"],
                "guitar": ["guitar picks", "guitar strings", "capo", "tuner", "guitar bag"],
                ...
                "headphones": ["headphone stand", "carry case", "extra ear pads", "DAC", "cable extension"],
                "tablet": ["tablet case", "stylus pen", "screen protector", "bluetooth keyboard", "stand"],
                "gaming": ["gaming mouse", "mousepad", "headset", "controller", "RGB LED strip"],
                "gym": ["gym bag", "protein shaker", "resistance bands", "workout gloves", "foam roller"],
                "kitchen": ["cutting board", "knife set", "measuring cups", "storage containers", "apron"],
                ...
                "screwdriver": ["screw set", "drill bits", "pliers", "electrical tape", "tool box"],
                "tool": ["tool box", "safety goggles", "measuring tape", "work gloves", "cable ties"],
                "shoe": ["shoe polish", "insoles", "shoe rack", "socks", "shoe cleaner"],
                "sneaker": ["sneaker cleaner", "shoe rack", "extra laces", "sports socks", "shoe bag"],
            }
            accessories = None
            for key, items in accessory_map.items():
                if key in base_product:
                    accessories = items
                    break
            if not accessories:
                # Smart fallback — search for related products
                accessories = [f"{base_product} case", f"{base_product} accessories", f"{base_product} stand", f"{base_product} cover", f"{base_product} charger"]
            
            response_text = f"Great picks to go with your {base_product}:"
            return {
                "type": "question",
                "text": response_text,
                "options": accessories[:5],
                "products": None,
                "metadata": {"confidenceScore": 1.0, "extractedAttributes": {"_action": "companion_search", "_search_term": base_product}, "questionCount": 0},
            }
...
def process_user_message(session: Session, text: str) -> dict:
    ...
    _is_random = (
        ...
        (len(_cleaned_lower) >= 5 and _ratio > 4 and not any(w in _cleaned_lower for w in ["gym", "dry", "fly", "try", "cry", "fry", "sky"])) or
