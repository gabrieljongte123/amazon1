...
def search_and_recommend(search_term: str, attributes: dict[str, Any] | None = None, max_price: float | None = None, min_rating: float | None = None) -> list[dict]:
    ...
    # ═══ PRODUCT TITLE SAFETY GUARD ═══
    # If product_name still looks like a sentence or question (contains verbs, 
    # question words, pronouns, or is too many words), it is NOT a valid product name.
    # Return None to signal caller should ask clarification instead.
    _sentence_words = {
        "what", "when", "where", "who", "why", "how", "should", "would", "could",
        "feel", "feeling", "today", "sunny", "hungry", "bored", "going", "wearing",
        "suggest", "bought", "well", "along", "with", "that", "this", "which",
        "would", "could", "shall", "might", "must", "have", "has", "had",
    }
    _name_words = set(product_name.lower().split())
    _sentence_overlap = _name_words & _sentence_words
    _word_count = len(product_name.split())
    
    # Reject if: has sentence words, OR more than 4 words with no ontology match
    _is_sentence_like = (
        len(_sentence_overlap) >= 1 or
        (_word_count > 4 and (not classification or classification.confidence == 0))
    )
    
    if _is_sentence_like:
        return None  # Caller will ask clarification
